...
while [[ "$#" -gt 0 ]]; do
	case "$1" in
		...
		-s|--superrunfolder) SuperRunFolder="$2"; [ "$(echo "$2" | cut -c 1)" == "-" ] || [ "$2" == "" ] && help || shift 2;;
        -m|--mainf) MainF="$2"; [ "$(echo "$2" | cut -c 1)" == "-" ] || [ "$2" == "" ] && help || shift 2;;
        -g|--geno) GenotypeCalls="$2"; [ "$(echo "$2" | cut -c 1)" == "-" ] || [ "$2" == "" ] && help || shift 2;;
		-t|--typetier) typeTier="$2"; [ "$(echo "$2" | cut -c 1)" == "-" ] || [ "$2" == "" ] && help || shift 2;;
		-l|--reflist) RefList="$2"; [ "$(echo "$2" | cut -c 1)" == "-" ] || [ "$2" == "" ] && help || shift 2;;
...
#GenotypeCalls=/home/pato/Skrivebord/HPV_subtyping/GenotypeCalls/"${SuperRunFolder}"
...
RefdF=$MainF/ReferenceDetails; RefF=$MainF/References; QCF=$MainF/QC; ErrorF=$QCF/Errors; ResultsF=$MainF/Results/$SuperRunFolder

###################### Find genotypecall ref ##########################

# HER DEFINERES REFERENCE FUNDET AF KOMBINEREDE REFERENCER MODUL
AllGeno=$(< $GenotypeCalls/${FastQFile%_filt}_T${typeTier}_SplitTo.txt) 
# RefList=$(< $SeqF/TopRunName/SubtypeCallFromCov.txt) # Uncomment hvis der skal bruges custom referencer fra customRefGenerator.sh
...
touch $ResultsF/$FastQFile/MismatchCounts_"${FastQFile}".txt
touch $ResultsF/$FastQFile/MismatchCounts_filt_"${FastQFile}".txt
MMcountFile=$ResultsF/$FastQFile/MismatchCounts_"${FastQFile}".txt
MMcountFile_filt=$ResultsF/$FastQFile/MismatchCounts_filt_"${FastQFile}".txt # Filtered MM count file
...
for refType in "${RefList}"; do 
	...
	echo Reference is now "${refType}" in splitRef variant calling
...
#	rm -r $RefF
#fi
rm -f $RefdF/RefSubtyper_"${FastQFile}".txt