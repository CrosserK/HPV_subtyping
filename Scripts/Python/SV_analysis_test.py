...
ReferenceList = ["deletions_d_12_100"]
MeanLengthList = ["1000"]
TotalBasesList = ["500000000"]
AlignmentList = ["Minimap2"]
SVCallerList = ["Sniffles"]
threads=12
# SV caller settings:
NumbOfReadsForCallList = ["3"]
MinLengthForSVList = ["30"]
Minmapping_qualList = ["50"]




# Tæller hvor mange der ikke kunne læses
fails = 0

for refType in ReferenceList:
  for meanLength in MeanLengthList:
    for total_bases in TotalBasesList:
      for aligner in AlignmentList:
        for SVCaller in SVCallerList:
          for NumbOfReadsForCall in NumbOfReadsForCallList:
            for MinLengthForSV in MinLengthForSVList:
              for Minmapping_qual in Minmapping_qualList:
